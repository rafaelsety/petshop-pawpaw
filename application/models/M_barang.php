...
class M_barang extends CI_Model
{
	...
	{
		$query = $this->db->get_where($this->_table, ['kd_produk' => $kode_barang]);
		return $query->row();
	}
	...
	public function lihat_nama_barang($nama_barang)
	{
		...
		$query = $this->db->where(['nama_produk' => $nama_barang]);
		$query = $this->db->get($this->_table);
		return $query->row();
	}
	...
	{
		$query = $this->db->select('*');
		$query = $this->db->where(['kd_produk' => $kode_barang]);
		$query = $this->db->get($this->_table);
		return $query->row();
	}
	...
	public function min_stok($stok, $nama_barang)
	{
		...
		$query = $this->db->where('kd_produk', $nama_barang);
